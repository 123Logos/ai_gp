...
public final class AiChatIntentSignals {
	...
	private static final Pattern POSITIVE_TOOL_USE = Pattern.compile(
			"(?i)(应|需要|必须|建议)(调用|使用)\\s*(任务工具|create_task|list_tasks|update_task|delete_task|get_task)"
					+ "|应调用\\s*(create_task|list_tasks|update_task|delete_task|get_task)"
					+ "|须调用\\s*(create_task|list_tasks)"
					+ "|是否[^\\n]{0,24}(调用|使用)[^\\n]{0,16}工具[^\\n]{0,12}[：:][^\\n]*是");
